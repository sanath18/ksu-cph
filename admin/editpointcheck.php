...
if(!empty($tmp_name)){
  ...
  $s_loc=$_SESSION['path'];
	...
	$insert_query = "update `health_location` set Title = '$title',Latitude='$latitude',Longitude='$longitude',LocationType=$optradio,url='$url',Description='$description',UserId=$user_id,Approval=5,color='#000000',size='medium',icon='n' where health_location.LocationId=$LocationId";   
  if(mysqli_query($conn,$insert_query));{
    $insert_picture= "update `health_picture` set Path=$s_loc where LocationId=$LocationId";
    mysqli_query($conn,$insert_picture);
   header("location: editquestion.php?LocationId=".$LocationId."&Locationtype=".$optradio);
  }
unset($_SESSION['path']);
mysqli_close($conn);
}
